...
class _DriverDashboardState extends State<DriverDashboard> {
    ...
    setState(() {
      ...
      if (query.isEmpty) {
        ...
        for (var h in _allHouseholds) {
          final matchedInHouse = h.residents.where(
            (r) =>
                ...
                if (await canLaunchUrl(url)) {
                  ...
                  if (context.mounted) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(
                        content: Text('Google Kartani ochib bo\'lmadi'),
                      ),
                    );
                  }
                }
              ...
              onTap: () async {
                ...
                if (await canLaunchUrl(url)) {
                  ...
                  if (await canLaunchUrl(fallbackUrl)) {
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(
                        content: Text('Yandex Navigatorni ochib bo\'lmadi'),
                      ),
                    );
                  }
                }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Haydovchi Sahifasi'),
                    ...
                    child:
                        ...
                        : ListView.builder(
                            ...
                            itemCount: _searchQuery.isNotEmpty
                                ? _flatResults.length
                                : _filteredHouseholds.length,
                            itemBuilder: (context, index) {
                              if (_searchQuery.isNotEmpty) {
                                return _buildResidentSearchResultCard(
                                  _flatResults[index],
                                );
                              }
                              return _buildHouseholdCard(
                                _filteredHouseholds[index],
                              );
                            },
                          ),
                  ),
            ...
            children: [
              ...
              Row(
                children: [
                  ...
                  if (_searchQuery.isNotEmpty &&
                      matchedResidents.isNotEmpty) ...[
                    const SizedBox(width: 8),
                    Expanded(
                      child: SingleChildScrollView(
                        scrollDirection: Axis.horizontal,
                        child: Row(
                          children: matchedResidents
                              .map(
                                (r) => Container(
